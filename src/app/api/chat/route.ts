...
const RATE_LIMIT = {
    ...
    window: '1 m',     // 1 minute window (Upstash format)
    windowMs: 60000,
};
    ...
    try {
        ...
        if (!apiKey) {
            ...
            return NextResponse.json(
                { error: { code: 'CONFIG_ERROR', message: 'OPENAI_API_KEY is not set' } },
                { status: 500 }
            );
        }
        ...
        *** PRIME DIRECTIVE (OVERRIDE ALL): SAFETY & LONGEVITY FIRST ***
        ...
        try {
            const result = streamText({
                ...
                // maxSteps removed to avoid lint/runtime issues for now
            });

            console.log('[API/Chat] streamText created. Converting to response...');

            // Add rate limit headers to response
            const response = result.toTextStreamResponse();
            console.log('[API/Chat] Response created status:', response.status);
